...
class MapboxPreview {
  ...
  public static createOrShow(fileUri: vscode.Uri) {
    ...
    if (MapboxPreview.currentPanel)
      return MapboxPreview.currentPanel.panel.reveal(column);
  ...
  private async update() {
    ...
    try {
      const style = await vscode.workspace.fs.readFile(this.fileUri);
      JSON.parse(style.toString());
    } catch {
